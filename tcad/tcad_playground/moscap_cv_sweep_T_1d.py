...
# =========================================================
#  MOSCAP C–V (1D) T スイープ
#   - 非線形 Poisson（可変誘電率）
#   - 温度 T を複数値でスイープ
#   - 図保存：fig/moscap_cv_T_sweep.png
# =========================================================
...
FIG_DIR  = "fig"
FIG_NAME = "moscap_cv_T_sweep.png"
...
ni = 1e16                 # [1/m^3] ここでは T に依らず一定とする
...
def charge_density(psi, Nd_minus_Na, Vt):
    """温度依存の Vt を引数で受け取る"""
...
def solve_nonlinear(Vg, psi_init, x, eps, Nd_minus_Na, Vt):
    psi_right = 0.0
    psi = psi_init.copy()

    for it in range(max_iter):
        rho = charge_density(psi, Nd_minus_Na, Vt)
...
def compute_cv_for_T(Tval):
    ...
    # 初期値（Vg = 0 の線形 Poisson）: T 依存なしと近似
    ...
    for Vg in Vg_list:
        psi = solve_nonlinear(Vg, psi, x, eps, Nd_minus_Na, Vt)
        rho = charge_density(psi, Nd_minus_Na, Vt)
...
for Tval in T_LIST:
    Vg, C = compute_cv_for_T(Tval)
    label = f"T = {Tval:.0f} K"
    plt.plot(Vg, C, marker="o", label=label)

plt.xlabel("Vg [V]")
plt.ylabel("C [μF/cm$^2$]")
plt.title("MOSCAP C–V (1D, T sweep)")
plt.grid(True)
plt.legend()
plt.tight_layout()
